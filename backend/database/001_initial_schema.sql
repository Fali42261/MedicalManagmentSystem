...
BEGIN
 CREATE TABLE dbo.Stores(Id BIGINT IDENTITY(1,1) PRIMARY KEY,Name NVARCHAR(160) NOT NULL,Phone NVARCHAR(18) NOT NULL,Address NVARCHAR(300) NULL,City NVARCHAR(100) NOT NULL,State NVARCHAR(100) NOT NULL,IsPrimary BIT NOT NULL DEFAULT(0),IsActive BIT NOT NULL DEFAULT(1),IsDeleted BIT NOT NULL DEFAULT(0),CreatedByUserId BIGINT NOT NULL,CreatedAtUtc DATETIME2 NOT NULL DEFAULT(SYSUTCDATETIME()),RowVersion ROWVERSION NOT NULL,CONSTRAINT FK_Stores_User FOREIGN KEY(CreatedByUserId) REFERENCES dbo.Users(Id)); CREATE UNIQUE INDEX UX_Stores_Name_Active ON dbo.Stores(Name) WHERE IsDeleted=0;
END;
IF OBJECT_ID('dbo.BackupSequence','SO') IS NULL CREATE SEQUENCE dbo.BackupSequence AS BIGINT START WITH 1 INCREMENT BY 1;
IF OBJECT_ID('dbo.Backups','U') IS NULL
BEGIN
 CREATE TABLE dbo.Backups(Id BIGINT IDENTITY(1,1) PRIMARY KEY,BackupNumber NVARCHAR(40) NOT NULL UNIQUE,Type NVARCHAR(30) NOT NULL,SizeBytes BIGINT NOT NULL DEFAULT(0),Status NVARCHAR(20) NOT NULL,IsDeleted BIT NOT NULL DEFAULT(0),CreatedByUserId BIGINT NOT NULL,CreatedAtUtc DATETIME2 NOT NULL DEFAULT(SYSUTCDATETIME()),CONSTRAINT FK_Backups_User FOREIGN KEY(CreatedByUserId) REFERENCES dbo.Users(Id));
END;
IF OBJECT_ID('dbo.AppSettings','U') IS NULL
BEGIN
 CREATE TABLE dbo.AppSettings([Key] NVARCHAR(80) NOT NULL PRIMARY KEY,[Value] NVARCHAR(500) NOT NULL,IsDeleted BIT NOT NULL DEFAULT(0),UpdatedByUserId BIGINT NOT NULL,UpdatedAtUtc DATETIME2 NOT NULL DEFAULT(SYSUTCDATETIME()),CONSTRAINT FK_AppSettings_User FOREIGN KEY(UpdatedByUserId) REFERENCES dbo.Users(Id));
END;
