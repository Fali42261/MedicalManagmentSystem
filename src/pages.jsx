...
export function Compliance() {
  ...
  return <><PageHeader title="GST & Compliance" description="Monitor GST summaries, e-invoices and filing readiness."><Button variant="secondary" icon="download">Export GST data</Button><Button icon="check">Validate entries</Button></PageHeader><div className="compliance-banner"><span><Icon name="check" size={22}/></span><div><b>Books are GST-ready</b><p>All transactions through 12 Sep 2026 are validated. No tax mismatch detected.</p></div><Badge tone="success">Healthy</Badge></div><div className="report-stats"><div><span>Taxable sales</span><b>₹4,12,450</b><small>September 2026</small></div><div><span>Output GST</span><b>₹32,180</b><small>Collected</small></div><div><span>Input GST</span><b>₹24,620</b><small>Available credit</small></div><div><span>Net payable</span><b>₹7,560</b><small>Due 20 Sep</small></div></div><Panel title="Compliance overview"><DataTable headers={['Return / Service','Description','Period','Due / Usage','Status','Action']}>{filings.map(row=><tr key={row[0]}><td><b>{row[0]}</b></td><td>{row[1]}</td><td>{row[2]}</td><td>{row[3]}</td><td><Badge tone={row[4]==='Due soon'?'warning':'success'}>{row[4]}</Badge></td><td><button className="text-button">View details</button></td></tr>)}</DataTable></Panel></>
}
...
export function UsersRoles({ showToast }) {
  ...
  return <><PageHeader title="Users & Roles" description="Control user access, store permissions and billing powers."><Button icon="plus" onClick={()=>showToast('User invitation flow opened')}>Invite user</Button></PageHeader><div className="roles-grid"><div><span><Icon name="users"/></span><b>Administrator</b><small>Full access · 1 user</small></div><div><span><Icon name="receipt"/></span><b>Billing operator</b><small>Sales access · 1 user</small></div><div><span><Icon name="box"/></span><b>Inventory manager</b><small>Stock access · 1 user</small></div><div><span><Icon name="chart"/></span><b>Accountant</b><small>Accounts access · 1 user</small></div></div><Panel title="User directory"><DataTable headers={['User','Email','Role','Store access','Status','Last login','']}>{users.map(row=><tr key={row[0]}><td><div className="user-cell"><span>{row[1][0]}</span><div><b>{row[1]}</b><small>{row[0]}</small></div></div></td><td>{row[2]}</td><td>{row[3]}</td><td>{row[4]}</td><td><Badge tone={row[5]==='Active'?'success':'neutral'}>{row[5]}</Badge></td><td>Today, 10:24 AM</td><td><button className="icon-button"><Icon name="edit" size={16}/></button></td></tr>)}</DataTable></Panel></>
}

export function DataTools({ showToast }) {
  return <><PageHeader title="Data & Backup" description="Protect store data and move records safely."><Button icon="download" onClick={()=>showToast('Backup created successfully')}>Create backup</Button></PageHeader><div className="data-cards"><div><span className="data-icon"><Icon name="box"/></span><div><b>Cloud backup</b><p>Automatic encrypted backup every day at 11:30 PM.</p><Badge tone="success">Up to date</Badge></div><Button variant="secondary">Configure</Button></div><div><span className="data-icon"><Icon name="download"/></span><div><b>Import data</b><p>Import medicine, supplier and opening stock data from Excel.</p><small>XLSX and CSV supported</small></div><Button variant="secondary">Start import</Button></div><div><span className="data-icon"><Icon name="return"/></span><div><b>Export business data</b><p>Download master and transaction records for archiving.</p><small>Excel or JSON format</small></div><Button variant="secondary">Export data</Button></div><div><span className="data-icon"><Icon name="alert"/></span><div><b>Audit trail</b><p>Review edits, deleted records and operator activity.</p><small>248 events this month</small></div><Button variant="secondary">View activity</Button></div></div><Panel title="Backup history"><DataTable headers={['Backup','Created','Type','Size','Created by','Status','']}><tr><td><b>backup-2026-09-12</b></td><td>12 Sep, 11:30 PM</td><td>Automatic</td><td>24.8 MB</td><td>System</td><td><Badge tone="success">Completed</Badge></td><td><button className="text-button">Download</button></td></tr><tr><td><b>backup-2026-09-11</b></td><td>11 Sep, 11:30 PM</td><td>Automatic</td><td>24.3 MB</td><td>System</td><td><Badge tone="success">Completed</Badge></td><td><button className="text-button">Download</button></td></tr></DataTable></Panel></>
}
