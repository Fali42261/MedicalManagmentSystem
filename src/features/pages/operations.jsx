...
export function Returns({ showToast, permissions }) {
  const { data: { returns, invoices }, mutations } = usePharmacyData(); const [creating,setCreating]=useState(false); const [sale,setSale]=useState(null); const [reason,setReason]=useState('Customer request'); const [qty,setQty]=useState({}); const [saving,setSaving]=useState(false)
  const submit=async e=>{e.preventDefault();if(!sale)return;const items=sale.lines.filter(l=>Number(qty[l.id])>0).map(l=>({saleItemId:l.id,quantity:Number(qty[l.id])}));if(!items.length){showToast('Select at least one quantity');return}setSaving(true);try{await mutations.createSaleReturn({saleId:sale.id,reason,items});setCreating(false);setSale(null);setQty({});showToast('Sales return completed and stock restored')}catch(x){showToast(x.message)}finally{setSaving(false)}}
  return <><PageHeader title="Returns" description="Process sales returns with stock restoration and audit trail.">{permissions?.has('Add')&&<Button icon="plus" onClick={()=>setCreating(true)}>Create sales return</Button>}</PageHeader>{creating&&<Panel title="Create sales return" className="inline-form-panel"><form className="workflow-form workflow-form--wide" onSubmit={submit}><label>Sales invoice<select required value={sale?.id||''} onChange={e=>{const x=invoices.find(i=>String(i.id)===e.target.value);setSale(x);setQty({})}}><option value="">Select invoice</option>{invoices.filter(i=>i.status!=='Cancelled').map(i=><option key={i.id} value={i.id}>{i.code} · {i.customer||'Walk-in'} · {money(i.total)}</option>)}</select></label><label>Reason<select value={reason} onChange={e=>setReason(e.target.value)}><option>Customer request</option><option>Damaged</option><option>Wrong item</option><option>Expired</option></select></label>{sale&&<div className="span-2"><DataTable headers={['Medicine','Batch','Sold qty','Return qty']}>{sale.lines.map(l=><tr key={l.id}><td>{l.medicineName}</td><td>{l.batchNumber}</td><td>{l.quantity}</td><td><input type="number" min="0" max={l.quantity} value={qty[l.id]||''} onChange={e=>setQty({...qty,[l.id]:e.target.value})}/></td></tr>)}</DataTable></div>}<div className="row-actions"><Button variant="ghost" onClick={()=>setCreating(false)}>Cancel</Button><Button type="submit" icon="check" disabled={saving||!sale}>{saving?'Processing…':'Complete return'}</Button></div></form></Panel>}<Panel title="Return register"><DataTable headers={['Return no.','Type','Customer','Invoice','Date','Reason','Amount','Status']}>{returns.map(x=><tr key={x.id}><td><b className="primary-text">{x.code}</b></td><td>{x.type}</td><td>{x.party}</td><td>{x.invoice}</td><td>{new Date(x.date).toLocaleDateString('en-IN')}</td><td>{x.reason}</td><td><b>{money(x.amount)}</b></td><td><Badge tone="success">{x.status}</Badge></td></tr>)}</DataTable></Panel></>
}
