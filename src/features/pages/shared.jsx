...
export function MedicineModal({ item, onClose, onSave }) {
  ...
  const submit = async (event) => {
    ...
    const payload = { ...values, rowVersion: item?.rowVersion, purchase: Number(values.purchase), sale: Number(values.sale), stock: Number(values.stock), minStock: Number(values.minStock || 0) }
  ...
  return (
    <div className="modal-backdrop" onMouseDown={(e) => e.target === e.currentTarget && onClose()}>
      <form className="modal" role="dialog" aria-modal="true" aria-label={item ? 'Edit medicine' : 'Add medicine'} onSubmit={submit}>
        ...
        <div className="form-section"><h3>Batch, pricing and stock</h3><div className="form-grid"><label>Batch number<input name="batch" required defaultValue={item?.batch} placeholder="Batch no." /></label><label>Expiry date<input name="expiry" required type="month" defaultValue={item?.expiry?.slice(0, 7)} /></label><label>Purchase price<input name="purchase" required min="0" type="number" step="0.01" defaultValue={item?.purchase} placeholder="₹ 0.00" /></label><label>Sale price<input name="sale" required min="0" type="number" step="0.01" defaultValue={item?.sale} placeholder="₹ 0.00" /></label><label>Opening stock<input name="stock" required min="0" type="number" defaultValue={item?.stock} placeholder="0" /></label><label>Minimum stock<input name="minStock" min="0" type="number" defaultValue={item?.minStock} placeholder="10" /></label><label>GST rate<select name="gst" defaultValue={`${item?.gst || 12}%`}><option>5%</option><option>12%</option><option>18%</option></select></label><label>Rack number<input name="rack" defaultValue={item?.rack} placeholder="e.g. A-01" /></label></div>{error && <p className="form-error" role="alert">{error}</p>}</div>
